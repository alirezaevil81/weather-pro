/**
 * Weather Pro - Alpine Component Store
 * کامپوننت اصلی و وضعیت داشبورد هوشمند هواشناسی
 */
...
import {
    ...
    calculateDayDetails,
    calculateSafeSunExposure,
    getPressureTrendText,
    getMoonPhase,
    getSunProgress,
    getWeatherDetails,
    getAqiInfo
...
import {
    ...
    fetchWeatherAndAqiApi,
    fetchCompareCityWeatherApi
} from './api.js';

import {
    calculateHealthIndexes,
    checkWeatherAlerts,
    getClothingRecommendation,
    calculateActivities,
    calculateDay24Summary,
    getSmartSuggestion
} from './advisors.js';
...
export function weatherApp() {
    return {
        ...
        showCompareModal: false, showWindModal: false, showMoonModal: false, selectedHealthIndex: null, compareQuery: '', compareSuggestions: [], compareData: null, isCompareLoading: false,
        isRefreshing: false, theme: 'dark', currentLat: 35.6892, currentLon: 51.3890,
        liveTime: '', toast: { show: false, message: '' }, isDragging: false, isAutoScrolling: false, startX: 0, scrollLeft: 0,
        quickCities: [
            { name: 'تهران', admin: 'استان تهران', lat: 35.6892, lon: 51.3890 },
            { name: 'مشهد', admin: 'خراسان رضوی', lat: 36.2972, lon: 59.6067 },
            { name: 'اصفهان', admin: 'استان اصفهان', lat: 32.6546, lon: 51.6680 },
            { name: 'شیراز', admin: 'استان فارس', lat: 29.5918, lon: 52.5837 },
            { name: 'تبریز', admin: 'آذربایجان شرقی', lat: 38.0800, lon: 46.2919 },
            { name: 'رشت', admin: 'استان گیلان', lat: 37.2808, lon: 49.5832 },
            { name: 'کیش', admin: 'استان هرمزگان', lat: 26.5578, lon: 54.0194 },
            { name: 'اهواز', admin: 'استان خوزستان', lat: 31.3183, lon: 48.6706 },
            { name: 'کرج', admin: 'استان البرز', lat: 35.8327, lon: 50.9915 },
            { name: 'قم', admin: 'استان قم', lat: 34.6401, lon: 50.8764 }
        ],

        // Helpers
        vibrate, toFarsi, getAqiInfo,
        formatTemp(val) { return formatTemp(val, this.unit); },
        getIconSymbolId(iconClass) {
            if (!iconClass) return 'icon-circle-info';
            const cls = iconClass.toLowerCase();

            // Direct matches for clothing / advisor icons
            if (cls === 'vest' || cls.includes('fa-vest')) return 'icon-vest';
            if (cls === 'mitten' || cls.includes('fa-mitten')) return 'icon-mitten';
            if (cls === 'socks' || cls.includes('fa-socks')) return 'icon-socks';
            if (cls === 'shirt' || cls.includes('fa-shirt')) return 'icon-shirt';
            if (cls === 'hat-cowboy' || cls.includes('fa-hat-cowboy')) return 'icon-hat-cowboy';
            if (cls === 'umbrella' || cls.includes('fa-umbrella')) return 'icon-umbrella';
            if (cls === 'wind' || cls.includes('fa-wind')) return 'icon-wind';
            if (cls === 'glasses' || cls.includes('fa-glasses')) return 'icon-glasses';
            if (cls === 'mask' || cls.includes('fa-mask')) return 'icon-mask';

            if (cls.includes('fa-sun-plant-wilt')) return 'icon-sun';
            if (cls.includes('fa-sun')) return 'icon-sun';
            if (cls.includes('fa-cloud-sun-rain')) return 'icon-cloud-sun-rain';
            if (cls.includes('fa-cloud-sun')) return 'icon-cloud-sun-rain';
            if (cls.includes('fa-cloud-moon')) return 'icon-cloud-moon';
            if (cls.includes('fa-cloud-rain')) return 'icon-droplet';
            if (cls.includes('fa-cloud-showers-heavy')) return 'icon-water';
            if (cls.includes('fa-cloud-showers-water')) return 'icon-water';
            if (cls.includes('fa-cloud-arrow-up')) return 'icon-cloud-arrow-up';
            if (cls.includes('fa-cloud')) return 'icon-cloud-sun-rain';
            if (cls.includes('fa-snowflake')) return 'icon-snowflake';
            if (cls.includes('fa-bolt-lightning') || cls.includes('fa-bolt')) return 'icon-bolt';
            if (cls.includes('fa-brain')) return 'icon-brain';
            if (cls.includes('fa-lungs')) return 'icon-lungs';
            if (cls.includes('fa-bone')) return 'icon-bone';
            if (cls.includes('fa-seedling')) return 'icon-seedling';
            if (cls.includes('fa-leaf')) return 'icon-leaf';
            if (cls.includes('fa-person-biking') || cls.includes('fa-bicycle')) return 'icon-person-biking';
            if (cls.includes('fa-person-running')) return 'icon-person-running';
            if (cls.includes('fa-helicopter')) return 'icon-helicopter';
            if (cls.includes('fa-camera')) return 'icon-camera';
            if (cls.includes('fa-star')) return 'icon-star';
            if (cls.includes('fa-sailboat')) return 'icon-sailboat';
            if (cls.includes('fa-solar-panel')) return 'icon-solar-panel';
            if (cls.includes('fa-eye')) return 'icon-eye';
            if (cls.includes('fa-smog')) return 'icon-wind';
            if (cls.includes('fa-arrows-left-right')) return 'icon-arrows-rotate';
            if (cls.includes('fa-arrow-trend-up')) return 'icon-chart-line';
            if (cls.includes('fa-arrow-trend-down')) return 'icon-chart-line';
            if (cls.includes('fa-circle-half-stroke')) return 'icon-moon';
            if (cls.includes('fa-circle')) return 'icon-circle-info';
            if (cls.includes('fa-moon')) return 'icon-moon';
            if (cls.includes('fa-compass')) return 'icon-compass';
            if (cls.includes('fa-location-arrow')) return 'icon-location-arrow';
            if (cls.includes('fa-location-dot') || cls.includes('fa-map-pin')) return 'icon-location-dot';
            if (cls.includes('fa-trash-can')) return 'icon-trash-can';
            if (cls.includes('fa-fire') || cls.includes('fa-fire-flame-curved')) return 'icon-fire';
            if (cls.includes('fa-clock')) return 'icon-clock';
            if (cls.includes('fa-arrow-right')) return 'icon-arrow-right';
            if (cls.includes('fa-circle-notch')) return 'icon-spinner';
            if (cls.includes('fa-code-compare')) return 'icon-code-compare';
            if (cls.includes('fa-magnifying-glass')) return 'icon-magnifying-glass';
            if (cls.includes('fa-circle-xmark')) return 'icon-circle-xmark';
            if (cls.includes('fa-location-crosshairs')) return 'icon-location-crosshairs';
            if (cls.includes('fa-xmark')) return 'icon-xmark';
            if (cls.includes('fa-triangle-exclamation')) return 'icon-triangle-exclamation';
            if (cls.includes('fa-share-nodes')) return 'icon-share-nodes';
            if (cls.includes('fa-calendar-days')) return 'icon-calendar-days';
            if (cls.includes('fa-chart-line')) return 'icon-chart-line';
            if (cls.includes('fa-heart-pulse')) return 'icon-heart-pulse';
            if (cls.includes('fa-temperature-half')) return 'icon-temperature-half';
            if (cls.includes('fa-droplet')) return 'icon-droplet';
            if (cls.includes('fa-water')) return 'icon-water';
            if (cls.includes('fa-gauge-high')) return 'icon-gauge-high';
            if (cls.includes('fa-bottle-water')) return 'icon-bottle-water';
            if (cls.includes('fa-mountain-sun')) return 'icon-mountain-sun';
            if (cls.includes('fa-atom')) return 'icon-atom';
        ...
        async fetchWeather(lat, lon, cityName, provinceName = '') {
            ...
            try {
                ...
                this.weatherData = {
                    ...
                    clothing: getClothingRecommendation(
        ...
        startTouchDragging(e) {
            this.isDragging = true;
            this.isAutoScrolling = false;
            if (this.$refs.hourlyContainer && e.touches && e.touches[0]) {
                this.startX = e.touches[0].pageX - this.$refs.hourlyContainer.offsetLeft;
                this.scrollLeft = this.$refs.hourlyContainer.scrollLeft;
            }
        },
        stopDragging() {
        ...
        handleMouseMove(e) {
            ...
            e.preventDefault();
            const x = e.pageX - this.$refs.hourlyContainer.offsetLeft;
            const walk = (x - this.startX) * 1.5;
            this.$refs.hourlyContainer.scrollLeft = this.scrollLeft - walk;
        },
        handleTouchMove(e) {
            if (!this.isDragging || !this.$refs.hourlyContainer || !e.touches[0]) return;
            const x = e.touches[0].pageX - this.$refs.hourlyContainer.offsetLeft;
            const walk = (x - this.startX) * 1.5;
            this.$refs.hourlyContainer.scrollLeft = this.scrollLeft - walk;
        },
        scrollHourly(amount) {
        ...
        init() {
            ...
            window.addEventListener('mouseup', () => { if (this.isDragging) this.stopDragging(); });
            window.addEventListener('touchmove', e => { if (this.isDragging) this.handleTouchMove(e); }, { passive: false });
            window.addEventListener('touchend', () => { if (this.isDragging) this.stopDragging(); });
        }
    };
}
